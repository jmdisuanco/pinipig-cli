...
let writeFile = (targetpath, data, template) => {
  ...
  if (template.split('.')[1] != 'json') {
    fs.writeFileSync(path.join(files.getCurrentPath(), targetpath), prettier.format(result, option))
  } else {
    fs.writeFileSync(path.join(files.getCurrentPath(), targetpath), result)
  }
